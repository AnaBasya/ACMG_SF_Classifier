...
def strict_triage_and_output(
    ...
    *,
    ...
    def evaluate_gene_rules_for_variant(rules_text: Optional[str], vr: VariantRecord) -> Tuple[str, List[str]]:
        """
        Evaluate gene-specific rule block (from ACMG SF 'Rules' column) against a variant.
        
        The rules text may contain multiple lines with Include/Exclude conditions based on:
        - Impact (HIGH, MODERATE, LOW)
        - Consequence (e.g., "5_prime_UTR_variant")
        - HGVSp pattern (e.g., "p.Cys282Tyr")
        - HGVSp contains any of a list of protein changes.
        
        Args:
            rules_text (str): Raw rule text from ACMG SF table.
            vr (VariantRecord): Variant to evaluate.
